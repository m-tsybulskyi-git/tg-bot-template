...
public interface UsersProfileRepository extends CrudRepository<UserData, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE tg_bot_qa.user_data SET language = ?2 WHERE chat_id = ?1",
            nativeQuery = true)
    void setLanguageTag(long chatId, String languageTag);

    @Transactional
    @Modifying
    @Query(value = "UPDATE tg_bot_qa.user_data SET gender = ?2 WHERE chat_id = ?1",
            nativeQuery = true)
    void updateGender(long chatId, String languageTag);

    @Transactional
    @Modifying
    @Query(value = "UPDATE tg_bot_qa.user_data SET age = ?2 WHERE chat_id = ?1",
            nativeQuery = true)
    void updateAge(long chatId, int age);

    @Transactional
    @Modifying
    @Query(value = "UPDATE tg_bot_qa.user_data SET state = ?2 WHERE chat_id = ?1",
            nativeQuery = true)
    void setBotState(long chatId, String state);

    @Transactional
    @Query(value = "SELECT language FROM tg_bot_qa.user_data WHERE chat_id = ?1",
            nativeQuery = true)
    String getLanguageTag(long chatId);

    @Transactional
    @Query(value = "SELECT state FROM tg_bot_qa.user_data WHERE chat_id = ?1",
            nativeQuery = true)
